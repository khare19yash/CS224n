...
class ParserModel(Model):
    ...
    def add_placeholders(self):
        ...
        self.droput_placeholder = tf.placeholder(dtype=tf.float32)
        ### END YOUR CODE
    ...
    def add_embedding(self):
        ...
        embeddings = tf.nn.embedding_lookup(embedding_matrix,self.inputs_placeholder,name='embeddings')
    ...
    def add_prediction_op(self):
        ...
        shape1 = (self.conig.n_features, self.config.hidden_size)
        shape2 = (self.config.hidden_size,self.config.n_classes)
        W = tf.Varaible(xavier_initializer(shape1),name='W')
        U = tf.Varaible(xavier_initializer(shape2),name='U')
        b1 = tf.Variable(tf.zeros([self.config.hidden_size]),name='b1')
        b2 = tf.Variable(tf.zeros([self.config.n_classes]),name='b2')
        z = tf.matmul(self.inputs_placeholder,W) + b1
        h = tf.nn.relu(z)
